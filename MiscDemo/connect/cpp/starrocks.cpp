...
bool StarRocksClient::init(const string& host, const string& user, const string& passwd,
...
bool StarRocksClient::exec(const string& sql) {
...
int main() {
    // StarRocks connection host
    string host = "127.0.0.1";
    // StarRocks connection port
    int port = 9030;
    // StarRocks connection username
    string user = "root";
    // StarRocks connection password
    string password = "";
    // Local mysql sock address
    string sock_add = "/var/lib/mysql/mysql.sock";
    // database to create
    string database = "cpp_starrocks";
    ...
    StarRocksClient client_new;
    // connect to StarRocks
    client_new.init(host, user, password, database, port, sock_add);
    std::cout << "init new Client" << std::endl;

    // create StarRocks table
    string sql_create_table = "CREATE TABLE cpp_starrocks_table(siteid INT,citycode SMALLINT,pv BIGINT SUM) "\
    ...
    // insert into StarRocks table
    string sql_insert = "insert into cpp_starrocks_table values(1, 2, 3), (4,5,6), (1,2,4)";
    std::cout << sql_insert << std::endl;
    client_new.exec(sql_insert);

    // select from StarRocks table
    string sql_select = "select * from cpp_starrocks_table";
